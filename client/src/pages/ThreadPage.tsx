  ...
  return (
    <>
      <div className="overflow-hidden">
        <div
          className="fixed bg-black opacity-80 top-12 right-0 left-0 bottom-0"
          onClick={() => {
            navigate(-1);
          }}
        ></div>
          ...
          <div className="w-full h-fit px-4">
            <div className="mb-4">
              <div className="flex p-1.5 w-full text-xs">
                ...
                />
                <Link
                  to={`/c/${thread?.community.slug}`}
                  className="mr-0.5 font-bold hover:underline"
                >
                  c/{thread?.community.title}{" "}
                </Link>
                <span className="text-slate-500">
                  {" "}
                  • Posted by u/{thread?.username} {getTime(thread)} ago
                </span>
              </div>
              <div>
                <div className="pt-1">
                  <span className="antialiased text-lg font-medium">
                    {thread?.title}
                  </span>
                </div>
