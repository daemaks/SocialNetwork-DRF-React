...
export default function ThreadCreate() {
  ...
  const [locationState, setLocationState] = useState<CommunityProp | null>(
    null
  );
      ...
      <div className="w-[640px]">
        ...
        <div className="pb-5">
          <div className="flex items-center justify-center rounded bg-white border border-gray-300 p-3 max-w-max">
            <img
              className="rounded-full w-5 h-5 mr-1.5"
              src={locationState?.community.image}
            />
            <Link to={`/c/${locationState?.community.slug}`}>
              c/{locationState?.community.slug}
            </Link>
          </div>
        </div>
        <div className="w-full bg-white rounded p-3">
          ...
          <div className="flex flex-row-reverse">
            <button
              ...
              className="ml-2 py-1 px-2 border-2 rounded font-semibold"
            >
              Post
            </button>
            <button
              onClick={() => navigate(-1)}
              className="py-1 px-2 border-2 rounded font-semibold"
            >
              Cancel
            </button>
          </div>
